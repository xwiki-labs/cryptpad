...
var evictArchived = function (Env, cb) {
    ...
    // if they are older than the specified retention time
        ...
        blobs.list.archived.proofs(function (err, item, next) {
            ...
            //if (item && getNewestTime(item) > retentionTime) { return void next(); } // XXX
...
module.exports = function (Env, cb) {
    ...
    */
    ...
    var categorizeBlobsByActivity = function (w) { // unreliable metric...
        var n_blobs = 0;
        var active = 0;
        ...
        blobs.list.blobs(function (err, item, next) {
            ...
            /*
            if (getNewestTime(item) > inactiveTime) { // XXX
    ...
    categorizeBlobsByActivity = categorizeBlobsByActivity;
    ...
    // iterate over all accounts
        ...
        var pinAll = function (pinRef) {
            var times = pinRef.times;
            var pins = pinRef.pins;
            for (var id in times) {
                if (pins[id]) {
                    pinnedDocs.add(id);
                } else if (times[id] > inactiveTime) {
                    // consider docs active if they were pinned within the activity time
                    activeDocs.add(id);
                    //pinnedDocs.add(id);
                }
            }
        ...
        var accountIsActive = function (pinRef) {
            if (pinRef.latest > accountRetentionTime) {
                return true;
            }
    ...
    // if they have not been accessed within the specified retention time
        ...
        blobs.list.blobs(function (err, item, next) {
            ...
            //if (getNewestTime(item) > inactiveTime) { return void next(); } // XXX
