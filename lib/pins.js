...
var addPins = function (A, pins, times, date) {
    var l = A.length;
    var x;
    for (var i = 0; i < l; i++) {
        x = A[i];
...
var removePins = function (A, pins, times, date) {
    var l = A.length;
    var x;
    for (var i = 0; i < l; i++) {
        x = A[i];
    ...
    return function (line) {
        ...
        switch (l[0]) {
            case 'RESET': {
                pins = ref.pins = {};
                if (l[1] && l[1].length) {
                    addPins(l[1], ref.pins, times, date);
                }
            ...
            case 'UNPIN': {
                removePins(l[1], ref.pins, times, date);
                break;
            }
            default:
